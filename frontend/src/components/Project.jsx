...
const Project = () => {
  ...
  const cardWidth = 360; // 320px card + 40px gap (gap-10 = 2.5rem = 40px)
  const totalCards = projects.length;
  const totalWidth = cardWidth * totalCards + 5;

  // Start the animation immediately
  useEffect(() => {
    const updateSpeed = () => {
      let speed = 30; // default for desktops

      if (window.innerWidth < 640) {
        // Mobile view (sm)
        speed = 20; // <faster></faster>
  ...
  return (
    ...
    >
      <div className="max-w-8xl mx-auto px-6 text-center relative">
        ...
        <motion.div
          className="flex gap-10 cursor-pointer z-8"
          animate={controls}
